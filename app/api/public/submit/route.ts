...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc('submit_buyer_order', { p_token: token, p_items: normalized })
    ...
    if (botToken && chatId) {
      try {
        ...
        const { data: submission } = await supabase.from('buyer_submissions').select('id, order_id, total_quantity').eq('id', submissionId).single()
        const { data: order } = await supabase.from('buyer_orders').select('order_number, title').eq('id', submission?.order_id).single()
        const { data: submissionItems } = await supabase.from('buyer_submission_items').select('product_name, quantity').eq('submission_id', submissionId).order('created_at')
        ...
        if (telegramResponse.ok) {
          await supabase.from('buyer_submissions').update({ telegram_notified_at: new Date().toISOString() }).eq('id', submissionId)
        }
      } catch {
