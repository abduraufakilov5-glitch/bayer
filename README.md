...
1. Выполните `npm ci`.
2. Проект уже подключён к существующему Supabase `adviprrgbitenfubczfn`. При необходимости значения можно переопределить в `.env.local` через `NEXT_PUBLIC_SUPABASE_URL` и `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`.
3. В новом Supabase-проекте примените SQL-файлы в порядке:
   ...
   - `supabase/migrations/20260920122531_normalize_buyer_policies_and_indexes.sql`.
...
Без переменных окружения приложение продолжает использовать существующий Supabase проекта Bayer. На другом окружении задайте обе публичные переменные **до сборки**, чтобы переопределить подключение.

## Обновление существующей установки

1. Проверьте резервную копию базы и существующие данные на тестовом окружении.
2. Если первые две миграции уже применены, примените две датированные миграции `20260920084117_harden_order_workflow.sql` и `20260920122531_normalize_buyer_policies_and_indexes.sql`. Исправленная `001_init.sql` предназначена для новых установок; повторно выполнять её на рабочей базе не нужно.
