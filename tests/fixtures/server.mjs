...
const orders = [
 { id: token, owner_id: owner, title: 'Осенняя закупка', order_number: 12, public_token: token, status: 'waiting', created_at: '2026-09-20T10:00:00Z', updated_at: '2026-09-20T10:00:00Z', confirmed_at: null },
 { id: productId, owner_id: owner, title: 'Зимняя коллекция', order_number: 13, public_token: productId, status: 'received', created_at: '2026-09-19T10:00:00Z', updated_at: '2026-09-19T10:00:00Z', confirmed_at: null },
]
...
createServer(async (req, res) => {
 ...
 if (url.pathname === '/photo.svg') { res.setHeader('Content-Type','image/svg+xml'); res.end('<svg xmlns="http://www.w3.org/2000/svg" width="480" height="600"><rect width="480" height="600" fill="#ebe1d5"/><path d="M60 100h360v400H60z" fill="#82604e"/><path d="m60 100 360 400M420 100 60 500" stroke="#dfcbb4" stroke-width="40"/></svg>'); return }
 let data = {}
 if (url.pathname === '/auth/v1/user') data = user
 else if (url.pathname === '/auth/v1/token') data = { access_token: jwt, refresh_token: 'fixture-refresh', token_type: 'bearer', expires_in:3600, user }
   ...
   else data = { order: orders[0], products: [product] }
 }
 else if (url.pathname === '/rest/v1/buyer_orders') data = url.searchParams.has('id') ? orders[0] : orders
