...
test('fresh migrations, buyer isolation, immutable publication and atomic/idempotent confirmation', async () => {
  ...
  try {
    ...
    await assert.rejects(db.query("update public.buyer_orders set status='waiting' where id=$1", [order.id]), /priced products/)
    const product = (await db.query<{id:string}>("insert into public.buyer_products(order_id,name,price,image_path) values ($1,'Платок',56.4,$2) returning id", [order.id, owner+'/scarf.jpg'])).rows[0]
    ...
    await assert.rejects(db.query('delete from public.buyer_orders where id=$1',[order.id]), /cannot be deleted/)
    ...
    const payload = [{ product_id: product.id, quantity: 3 }]
